...
def _load_domain_cache() -> dict[str, dict]:
    """Load the domain cache from disk. Returns {domain: {tickers: [...], screen_results: {...}, metrics: {}}}."""
...
def run_funnel(tickers: list[str], portfolio_myr: float, use_db: bool = False) -> None:
    ...
    for domain, domain_tickers in domains.items():
        ...
        if cached_tickers == domain_tickers:
            # Domain unchanged — reuse cached screen results and metrics
            ...
            for t in domain_tickers:
                ...
                if t in cached_metrics:
                    metrics_cache[t] = cached_metrics[t]
            ...
            logger.info("Domain '%s': changed (added=%d, removed=%d) — will scrape %d tickers",
                        domain, len(added), len(removed), len(domain_tickers))
        ...
        for domain in domains_to_scrape:
            ...
            for t, mlist in domain_metrics.items():
                domain_metrics_ser[t] = [
                    {"year": m.year, "revenue": m.revenue, "net_income": m.net_income,
                     ...
                     "equity": m.equity, "shares_outstanding": m.shares_outstanding,
